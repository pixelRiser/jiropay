...
class AuthController extends Controller
{
    public function register(Request $request): JsonResponse
    {
        $validated = $request->validate([
            ...
            // Pas de confirmation — un seul champ mot de passe, comme le formulaire d'origine.
    ...
    {
        // L'envoi du lien fait partie de la même transaction que la création :
        // si sendResetLink() échoue (ex: notification mal configurée), tout est
        // annulé plutôt que de laisser un compte orphelin, impossible à activer.
        return DB::transaction(function () use ($data, $guichetId) {
            $user = User::create([
                'name'     => $data['name'],
                'email'    => $data['email'],
